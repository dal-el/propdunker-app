...
type OppDefPayload = { source?: any; stats?: Record<string, { standard?: OppDefTable; percentage?: OppDefTable }> };
...
function resolveTeamKeyFuzzy(raw: any): TeamKey | null {
  ...
  const strip = (x: string) =>
    x
      ...
      .replace(/\b(beko|aktor|rapyd|meridianbet|mozzart|bet|segafredo|emporio|armani|ea7|ldlc|ibi)\b/g, " ")
      .replace(/\s+/g, " ")
      .trim();
  ...
  for (const [k, v] of Object.entries(TEAM_CANONICAL as any)) {
    ...
    else if (dispStripped && normStripped && (dispStripped.includes(normStripped) || normStripped.includes(dispStripped)))
      score = 70;
    else {
...
function OppDefPosPill({ row }: { row: BetLine }) {
  ...
  return (
    <span
      className={clsx(PILL, "px-3 py-[4px] text-[11px]", oppToneClass(cell))}
...
function SidePill({ row }: { row: BetLine }) {
  ...
  return (
    <span className={clsx(PILL, "gap-2 px-3 py-[6px] text-[12px]")}>
...
function OddsPill({ odds }: { odds: number }) {
  return (
    <span className={clsx(PILL, "px-3 py-[6px] text-[12px] text-white/90")} title="Odds">
      {fmtOdds(odds)}
    </span>
  );
}
...
function PickTogglePill({ picked, onToggle }: { picked: boolean; onToggle: React.MouseEventHandler<HTMLButtonElement> }) {
  return (
    <button
      type="button"
      onClick={onToggle}
  ...
  return (
    <button
      ...
      className={clsx(
        ...
        "px-3 py-[5px] text-[12px] transition-all duration-150 active:scale-95",
        active &&
          "border-blue-300/40 bg-blue-400/12 text-blue-200 ring-2 ring-blue-300/25 shadow-[0_10px_26px_rgba(0,0,0,0.35)]"
      )}
...
>(function BetRow({ row, open, onToggle, onPickHit, playerPropsActive = false, onTogglePlayerProps }, ref) {
  if (!row) return null;

  const logoSrcRaw = (row as any)?.team?.logo || (row as any)?.teamLogo || (row as any)?.logo || ((row as any)?.player)?.teamLogo || ((row as any)?.player)?.logo;
  const logoSrc = safeLogoUrl(logoSrcRaw);
  const propCategoryFull = ((row as any)?.prop)?.label || ((row as any)?.prop)?.fullName || ((row as any)?.prop)?.name || (row as any)?.propLabel || "";
  const bookmakerFull = (row as any)?.bookmakerFullName || (row as any)?.bookmakerName || (row as any)?.bookmaker || (row as any)?.book || (row as any)?.sportsbook || "";
  const teamKey = (row as any)?.__teamKey ?? (row as any)?.player?.teamKey ?? (row as any)?.player?.team_key ?? (row as any)?.player?.teamSlug ?? null;
  ...
  const onPickToggle: React.MouseEventHandler<HTMLButtonElement> = (e) => {
    e.preventDefault();
    e.stopPropagation();
    togglePick(pickRow as any);
  };

  const onPlayerPropsToggle: React.MouseEventHandler<HTMLButtonElement> = (e) => {
    e.preventDefault();
    e.stopPropagation();
    onTogglePlayerProps?.();
  };
    ...
    >
      ...
      >
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-1 mb-3 sm:mb-0">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 min-w-0 -translate-y-[4px]">
              ...
              <div className="min-w-0">
                <div className="flex items-baseline gap-2 min-w-0">
                  <span className="text-[15px] font-semibold tracking-wide truncate text-white/95">{row.player.name}</span>
                  <span className={clsx(PILL, "px-2 py-[2px] text-[11px] text-white/85")}>{row.player.pos}</span>
                </div>
              </div>
            ...
            <div className="mt-0 lg:hidden">
              <div className="grid grid-cols-[auto_1fr_auto] items-center gap-2 min-w-0">
                ...
                <div className="min-w-0 inline-flex items-center gap-2 justify-start">
                  {propCategoryFull ? (
                    <span className="min-w-0 max-w-[220px] text-[12px] font-medium text-white/85 truncate">{propCategoryFull}</span>
                  ) : (
            ...
            <div className="hidden lg:flex mt-2 items-center gap-2 min-w-0 flex-nowrap">
              ...
              <div className="min-w-0 inline-flex items-center gap-2 justify-start">
                {propCategoryFull ? (
                  <span className="min-w-0 max-w-[340px] text-[12px] font-medium text-white/85 truncate">{propCategoryFull}</span>
                ) : (
        ...
        <div className="hidden lg:grid grid-cols-[62px_62px_62px_62px_20px] gap-2 items-start">
          ...
          <div className="flex justify-center pt-[2px] opacity-70">{open ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}</div>
          {bookmakerFull || row.prop?.tier ? (
            <div className="col-span-4 mt-4 flex items-center justify-end gap-2 -translate-x-1 translate-y-1">
              {bookmakerFull ? <div className="text-[11px] leading-none text-white/60 whitespace-nowrap">{bookmakerFull}</div> : null}
        ...
        <div className="mt-2 flex items-end justify-between gap-3" onClick={(e) => e.stopPropagation()}>
          ...
          {bookmakerFull || row.prop?.tier ? (
            <div className="shrink-0 flex flex-col items-end justify-end">
              ...
              {bookmakerFull ? <div className="mt-1 text-[11px] leading-none text-white/60 whitespace-nowrap">{bookmakerFull}</div> : null}
            </div>
          ) : null}
        </div>
      </div>
