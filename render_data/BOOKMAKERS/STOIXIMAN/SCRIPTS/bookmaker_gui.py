...
from process_bookmaker_folder import process_files

class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("PropDunker — Bookmaker JSON Processor (STOIXIMAN)")
        self.geometry("820x520")
        self.resizable(False, False)

        self.files = []  # list of Paths

        tk.Label(self, text="Select one or more raw STOIXIMAN JSON files. When ready, press FINISHED.", font=("Segoe UI", 12)).pack(pady=12)
        ...
        tk.Label(self, text="Tip: Put all JSONs for a match (or all matches) in the same bookmaker folder and select them together.", font=("Segoe UI", 9)).pack(pady=4)

    def add_files(self):
        paths = filedialog.askopenfilenames(
            title="Select raw STOIXIMAN JSON files",
    ...
    def run(self):
        ...
        # If user selected a subset in the listbox, process only those; otherwise process all loaded.
        sel = self.listbox.curselection()
        if sel:
            chosen = [str(self.files[i]) for i in sel]
        else:
            chosen = [str(p) for p in self.files]
