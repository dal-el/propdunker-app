...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("PropDunker — Bookmaker JSON Processor (OPAP)")
        self.geometry("820x520")
        self.resizable(False, False)

        self.files = []

        tk.Label(self, text="Select one or more raw OPAP JSON files. When ready, press FINISHED.", font=("Segoe UI", 12)).pack(pady=12)
    ...
    def add_files(self):
        paths = filedialog.askopenfilenames(
            title="Select raw OPAP JSON files",
